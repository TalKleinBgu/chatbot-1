...
# If API keys are not submitted yet, show the input fields
if not st.session_state["api_keys_submitted"]:
    st.session_state["tavily_api_key"] = st.text_input("Enter Tavily API Key:", type="password")
    st.session_state["openai_api_key"] = st.text_input("Enter OpenAI API Key:", type="password")

    if st.button("Submit API Keys"):
        if st.session_state["tavily_api_key"] and st.session_state["openai_api_key"]:
            # Set environment variables for API keys
            os.environ["TAVILY_API_KEY"] = st.session_state["tavily_api_key"]
            os.environ["OPENAI_API_KEY"] = st.session_state["openai_api_key"]
            st.session_state["api_keys_submitted"] = True
        else:
            st.warning("Please enter both Tavily and OpenAI API keys to proceed.")
            st.stop()
...
if st.session_state["api_keys_submitted"]:
    ...
    def text_extraction_state(context):
        ...
        elif input_type == 'Upload File' and input_content:
            context['extracted_text'] = [input_content]
            pdf = context['pdf'][0]
            try:
    ...
    def analysis_state(context):
        ...
        if input_type == "Upload File":
            ...
            answer_template = (
                "based on this : {base_text} answer the query: {input_content} "
                "If the document is not a good source for the query "
                "Provide insights and answer about {input_content} with {search_results} and "
                "don't say based on the document or something close to this, "
                "also show in the end in nice way the sources from {sources_}"
            )
            ...
            answer_template = (
                "Provide insights about {search_results} and "
                "don't say based on the document or something close to this, "
                "also show in the end in nice way the sources from {sources_}"
            )
            answer_instructions = answer_template.format(
                sources_=sources_, search_results=search_results
            )
